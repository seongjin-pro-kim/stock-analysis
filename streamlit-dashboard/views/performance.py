...
def render():
    ...
    # 승리 — 프리뷰: 매도일 기준 종목, 수익율 7개 + 평균 RR, 기대값
    ...
    # Row 3
    cols = st.columns(4)
    # 승리 평균 소요일 — 프리뷰: 종목, 기대값 오름차순
    days_preview = _preview_table(
        [[r["name"], f"{r.get('expected_value', 0):.1f}"] for _, r in top_days_win.iterrows()],
        ["종목", "기대값"]
    )
    _kpi_with_preview(cols[0], "승리 평균 소요일", f"{avg_days_win:.1f}일", "", days_preview)
    _kpi_with_preview(cols[1], "패배 평균 소요일", f"{avg_days_loss:.1f}일", "")
    # 최대 낙폭 — 프리뷰: 종목, 손익비 오름차순
    dd_preview = _preview_table(
        [[r["name"], f"{r.get('rr_ratio', 0):.2f}"] for _, r in worst_dd.iterrows()],
        ["종목", "손익비"]
    )
        ...
        if "rr_ratio" in completed.columns and completed["rr_ratio"].notna().any():
            ...
            if rr_stats:
                ...
                fig3 = go.Figure(go.Bar(
                    ...
                    marker_color=["#14b8a6" if wr >= 70 else "#1a6b63" for wr in rrs["win_rate"]],
        ...
        if "expected_value" in completed.columns and completed["expected_value"].notna().any():
            ...
            if ev_stats:
                ...
                fig4 = go.Figure(go.Bar(
                    ...
                    marker_color=["#14b8a6" if wr >= 70 else "#1a6b63" for wr in evs["win_rate"]],
    ...
    fig5.add_trace(go.Scatter(
        ...
        yaxis="y2", line=dict(color="#22c55e", width=2), marker=dict(size=7),
    ))
    fig5.update_layout(
        plot_bgcolor="#0a0e14", paper_bgcolor="#0a0e14",
        height=300, margin=dict(l=0, r=40, t=10, b=0),
        ...
        xaxis=dict(color="#7a8599"),
        yaxis=dict(showgrid=True, gridcolor="#1e2530", color="#7a8599"),
        yaxis2=dict(overlaying="y", side="right", showgrid=False, color="#22c55e", range=[0, 110]),
        font=dict(color="#e2e8f0"), bargap=0.3,
    )
